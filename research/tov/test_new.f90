...
CONTAINS
   ...
   implicit none
     ...
     IF (x.EQ.0.d0) THEN
        rho2 = -4.d0*pi*y(2)*(1.d0 + kap*y(2))*(1.d0 + 3.d0*kap*y(2))   !-y(1)/(h*h*h)*b*e/d + a*b*((4.d0*pi*kap*y(2)*y(2)*h*h*h/y(1))*(3.d0/h - f(1)/y(1)))
        rho = y(2) + h*h*rho2/2.d0
        f(1) = 4.d0*pi*h*h*y(2)    !4.d0*pi*h*h*(y(2) + h*h*rho2/2.d0)
        !f(2) = -4.d0*g*y(2)**3*pi*h/&
        !        (3.d0*gma*p(y(2)))
        
        f(2) = -4.d0*g*y(2)*y(2)*y(2)*pi*h*(1.d0 + p(y(2))/(y(2)*c*c))*(1.d0 + 3.d0*p(y(2))/(y(2)*c*c))/&
                (3.d0*gma*p(y(2))*(1.d0 - 8.d0*g*y(2)*pi*h*h/(3.d0*c*c)))
        
        !(-2.d0*pi*y(2)/kap)*(h/3.d0 + h*h*h*rho2/10.d0)*(1.d0 + kap*rho)*&
   ...
   real*8, dimension(2)  :: k1, k2, k3, k4, j1, j2, j3, j4
     k1 = h*f(x, y)
